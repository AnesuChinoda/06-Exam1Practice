...
Authors: David Mutchler, Vibha Alangar, Valerie Galluzzi, Mark Hays,
         Amanda Stouder, their colleagues and Anesu Chinods.
...
def problem2a(circle, rectangle, window):
    ...
    window.render()
    window.continue_on_mouse_click()
    topright = rg.Point(rectangle.get_center().x + rectangle.get_width()/2, rectangle.get_center().y - rectangle.get_height()/2)
    bottomleft = rg.Point(rectangle.get_center().x - rectangle.get_width() / 2,
                        rectangle.get_center().y - rectangle.get_height() / 2)
    nes = rg.Line(topright, bottomleft)
    nes.attach_to(window)
    window.render()
    window.continue_on_mouse_click()
...
def problem2b(rect, n, delta, win):
    ...
    rect.attach_to(win)
    for k in range(n -1):
        nes = rg.Rectangle(rg.Point(rect.corner_1.x + (2*delta*(k+1)), rect.corner_1.y + (2*delta*(k+1))), rect.corner_2)
        nes.move_by(-delta*(k+1), -delta*(k+1))
        nes.attach_to(win)
    win.render()
